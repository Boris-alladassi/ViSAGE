...
multi_generations_selection <- function(initial_generation = NULL,
  ...
  for(i in 1:nGenerations){
    ...
    next_generation = AlphaSimR::setPheno(next_generation, H2 = tHet)
...
multi_generations_selection_sp <- function(initial_generation = NULL,
  ...
  for(i in 1:nGenerations){
    ...
    ngen_map <- AlphaSimR::getQtlMap(trait = 1, simParam = SP)
    ngen_qtls <- AlphaSimR::pullQtlGeno(next_generation, trait = 1, simParam = SP)
...
computeVariancePlot <- function(gen_list) {
  ...
  variance_df <- base::lapply(seq_along(gen_list), function(i) {
    ...
    tibble::tibble(
      generation = i,
      Additive = AlphaSimR::varA(pop),
      Dominance = AlphaSimR::varD(pop),
      AA_Epistasis = AlphaSimR::varAA(pop)
    )
  }) |>
    dplyr::bind_rows() |>
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(x = "Generation", y = "Prop. of Variance",
                  fill = "Variance Component", title = " ") +
    ggplot2::scale_fill_manual(
      values = c("Additive" = "#1b9e77", "Dominance" = "#d95f02",
                 "AA_Epistasis" = "#7570b3")) +
    custom_theme

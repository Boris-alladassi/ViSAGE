...
if(output.numerical) utils::write.table(GD,  "GAPIT.Genotype.Numerical.txt", quote = FALSE, sep = "\t", row.names = TRUE,col.names = NA)
...
stats::heatmap(as.matrix(bk$KW),  cexRow =.2, cexCol = 0.2, col=rev(grDevices::heat.colors(256)), scale="none", symkey=FALSE, trace="none")
grDevices::dev.off()
}
}
...
print("Exporting BLUP")
  try(utils::write.table(gs$BLUP, paste("GAPIT.", name.of.trait,".BLUP.csv" ,sep = ""), quote = FALSE, sep = ",", row.names = FALSE,col.names = TRUE))
}
...
print("debug end 2")
  ...
  if(file.output) utils::write.table(GWAS, paste("GAPIT.", name.of.trait, ".GWAS.Results.csv", sep = ""), quote = FALSE, sep = ",", row.names = FALSE,col.names = TRUE)
...
# stats::hclust() will perform the hiearchical cluster analysis
#cluster.distance.matrix <- stats::hclust(distance.matrix.stats::as.dist, method = kinship.cluster)
...
#utils::write.table(GAU, "GAU.txt", quote = FALSE, sep = "\t", row.names = TRUE,col.names = TRUE)
...
print("Manhattan ploting Chromosomewise")
  ...
  graphics::par(mar = c(5,5,4,3), lab = c(8,5,7))
  for(i in 1:numCHR)
  {
...
#print("Manhattan XY created")
  ...
  grDevices::pdf(paste("GAPIT.", name.of.trait,".Manhattan-Plot.Genomewise.pdf" ,sep = ""), width = 10)
 ...
 graphics::axis(1, at=ticks,cex.axis=1.5,labels=chm.to.analyze,tick=F)
  graphics::axis(2, at=1:y.lim,cex.axis=1.5,labels=1:y.lim,tick=F)
  graphics::box()
  grDevices::dev.off()
...
graphics::pie(variance,  col=colors, labels=labels,angle=45)
graphics::legend(1.0, 0.5, c("Genetic variance","Residual variance"), cex=1.5,
   fill=colors)

#Display the optimum compression
graphics::text(1.5,.0, "The optimum compression", col= "red")
for(i in 1:4){
graphics::text(1.5,-.1*i, theOptimum[i], col= "red")
}
grDevices::dev.off()
...
for(i in 1:length(unique(Compression[,1]))){
 ...
 #Make a legend
  #graphics::legend("topright",  label, fill = color.vector)
...
for(i in 1:length(unique(Compression[,1]))){
 ...
 #Make a legend
  #graphics::legend("topleft",  label, fill = color.vector)
...
# Make labels
      ...
      graphics::legend("topleft",  label, col = color.vector[line.color], lty = line.style, ncol=legend.col,horiz=FALSE)
...
colnames(KG)=kt.name
graphics::barplot(as.matrix(KG),  ylab= "Residual varaince", beside=TRUE, col=grDevices::rainbow(length(unique(Compression[,2]))))
...
if(inherits(BLUP.max, "try-error"))  return()
  ...
  #utils::write.table(KG, "Input_Matrix_for_GS_Heat_Map.txt", quote = FALSE, sep = "\t", row.names = FALSE,col.names = FALSE)

  grDevices::pdf(paste("GAPIT.", name.of.trait,".GPS.BLUPvsPEV", ".pdf", sep = ""),width = 9)
  #graphics::par(mfrow = c(1,1), mar = c(1,1,5,5), lab = c(5,5,7))
  graphics::par(mar = c(5,5,6,5))

  nba_heatmap <- stats::heatmap(KG, Rowv=NA, Colv=NA,  col =  rev(grDevices::heat.colors(256)),   scale="column",
...
graphics::par(mar = c(5,5,5,5))
stats::screeplot(PCA.X, type="lines")
grDevices::dev.off()

grDevices::pdf("GAPIT.PCA.pdf", width = 12, height = 12)
graphics::par(mar = c(5,5,5,5))
...
if(file.output) utils::write.table(PCs, "GAPIT.PCA.csv", quote = FALSE, sep = ",", row.names = FALSE,col.names = TRUE)
...
#grDevices::pdf(paste("GAPIT.LD.pdf",sep=""), width = 12, height = 12)
graphics::par(mar = c(25,25,25,25))
MyHeatmap <- try(LDheatmap(LDsnp, LDdist, LDmeasure="r", add.map=TRUE,
  SNP.name=LDsnpName,color=rev(grDevices::cm.colors(20)), name="myLDgrob", add.key=TRUE,geneMapLabelY=0.1) )
...
if(!inherits(MyHeatmap, "try-error")) {
  ...
  grid::grid.edit(grid::gPath("myLDgrob", "geneMap", "title"), gp=grid::gpar(just=c("center","bottom"), cex=0.8, col="black")) #Edit gene map title
...
#grid::grid.edit(grid::gPath("myLDgrob", "heatMap","title"), gp=grid::gpar(cex=1.0))   #Make title smaler
#grid::grid.edit(grid::gPath("myLDgrob", "geneMap", "title"), gp=grid::gpar(just=c("right","bottom"), cex=0.5, col="blue")) #Edit gene map title
#grid::grid.edit(grid::gPath("myLDgrob", "Key", "labels"), gp=grid::gpar(cex=.5, col="black"))  #edit key lable size and color
